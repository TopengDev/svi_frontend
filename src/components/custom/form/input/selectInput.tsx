...
export function SelectInput<TFormData>({
   ...
   useEffect(() => {
      if (!initialized && options?.length && multiple) {
         ...
         const selectedOptionStrings = selectedOptions.map((o) => o.value);
         ...
         setFormDataField(fieldName, (defaultValue as string) || '');
         setLoading(false);
      }
   }, [options?.length]);
                  ...
                  <div className="w-full">
                     ...
                     >
                        ...
                        {availableOptions?.length > 0 ? (
                           <div
                              className={cn(
                                 'z-10 w-full peer-checked:border-1 rounded-sm absolute top-full  left-0 bg-background  max-h-0 overflow-y-scroll',
                                 `${
